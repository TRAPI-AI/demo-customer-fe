...
const CarRentals = () => {
  ...
  const fetchAvailabilities = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/indie-campers-list-availabilities/${origin.toLowerCase()}/${destination.toLowerCase()}`,
        {
          params: {
  ...
  const handleBooking = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/indie-campers-create-a-booking",
        bookingData
      );
    ...
    <div>
      ...
      <ul>
        {availabilities.map((availability) => (
          <li
            key={availability.vehicle.identifier}
          ...
          >
            ...
            <div>
              ...
              <p className="capacity">
                Capacity: {availability.vehicle.max_capacity}
              </p>
              <p className="beds">Beds: {availability.vehicle.total_beds}</p>
              <p className="price-per-day">
                Price-per-day: {availability.price.cost_per_day}{" "}
                {availability.price.currency}
              </p>
              <p className="total-price">
                Total price:{" "}
                <span className="currency">
                  {availability.price.total_cost} {availability.price.currency}
                </span>
              </p>
            </div>
            <button
